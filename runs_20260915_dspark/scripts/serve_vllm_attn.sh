#!/bin/bash
# vLLM hidden-state server for the draft-attention comparison (MHA / GQA / MLA).
# Card 15, port 1123, hidden states into a dedicated scratch dir shared by all
# three trainers.
#
# Only difference vs serve_vllm.sh (the accum-comparison server): --max-num-seqs
# 8 -> 24. Three trainers x 4 workers = 12 in-flight requests, which would sit
# above the old 8-slot limit and turn into queueing; the slot count does not
# change the hidden states themselves, only how fast they are produced.
set -e
...
RUN_ROOT=/home/y50063564/processed_data/dspark_data/dspark_attn
HS_DIR=$RUN_ROOT/_server_hs
...
nohup vllm serve /home/y50063564/Qwen3-4B \
    ...
    > "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
echo "vLLM server log : $LOG_FILE"
echo "vLLM server pid : $(cat "$PID_FILE")"
