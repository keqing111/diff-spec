...
mkdir -p "$D/_hs_inorder0" "$D/logs"
LOG="$D/logs/vllm_diag_inorder0_$(date +%Y%m%d_%H%M%S).log"
nohup vllm serve /home/y50063564/Qwen3-4B --host 0.0.0.0 --port 1131 \
  ...
  --kv_transfer_config "{\"kv_connector\": \"ExampleHiddenStatesConnector\", \"kv_role\": \"kv_producer\", \"kv_connector_extra_config\": {\"shared_storage_path\": \"$D/_hs_inorder0\"}}" \
  --no-enable-chunked-prefill > "$LOG" 2>&1 &
echo $! > "$D/logs/vllm_diag_inorder0.pid"
echo "diag server log: $LOG  pid: $(cat $D/logs/vllm_diag_inorder0.pid)"
