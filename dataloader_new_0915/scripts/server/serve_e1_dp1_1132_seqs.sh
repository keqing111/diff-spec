...
mkdir -p "$D/_hs_e1_seqs" "$D/logs"
LOG="$D/logs/vllm_e1_$(date +%Y%m%d_%H%M%S).log"
nohup vllm serve /home/y50063564/Qwen3-4B --host 0.0.0.0 --port 1132 \
  --tensor-parallel-size 1 --seed 1024 --max-num-seqs ${MAX_NUM_SEQS:-8} --max-model-len 4096 \
  ...
  --kv_transfer_config "{\"kv_connector\": \"ExampleHiddenStatesConnector\", \"kv_role\": \"kv_producer\", \"kv_connector_extra_config\": {\"shared_storage_path\": \"$D/_hs_e1_seqs\"}}" \
  --max-num-batched-tokens ${MAX_NUM_BATCHED_TOKENS:-4096} \
  --no-enable-chunked-prefill > "$LOG" 2>&1 &
echo $! > "$D/logs/vllm_e1_seqs.pid"
echo "diag server log: $LOG  pid: $(cat $D/logs/vllm_e1_seqs.pid)"
