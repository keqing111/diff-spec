...
export DL_PROFILE_DIR=/home/y50063564/dataloader_diag/raw_w2_inorder1

D=/home/y50063564/dataloader_diag
mkdir -p "$D/logs" "$DL_PROFILE_DIR"
LOG="$D/logs/train_diag_w2_inorder1_$(date +%Y%m%d_%H%M%S).log"
nohup /usr/local/python3.12.13/bin/torchrun --standalone --nproc_per_node 4 \
  ...
  --save-path "$D/checkpoints_w2_inorder1" \
