...
export DL_PROFILE=1
export DL_PROFILE_DIR=/home/y50063564/dataloader_diag/raw

D=/home/y50063564/dataloader_diag
mkdir -p "$D/logs" "$DL_PROFILE_DIR"
LOG="$D/logs/train_diag_$(date +%Y%m%d_%H%M%S).log"
nohup /usr/local/python3.12.13/bin/torchrun --standalone --nproc_per_node 4 \
  ...
  --vllm-endpoint http://80.48.17.178:1130/v1 \
  --save-path "$D/checkpoints" \
