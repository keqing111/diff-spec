#!/bin/bash
# Group A: normal per-micro-batch parameter update  (grad_accum = 1).
# Trainer on card 13, hidden states served online by the card-15 vLLM server.
# Config is IDENTICAL to train_accum12.sh except --grad-accum (1 vs 12),
...
RUN_NAME=accum1
RUN_ROOT=/home/y50063564/processed_data/dspark_data/dspark_accum/accum1
...
nohup "$TORCHRUN" --standalone --nproc_per_node 1 \
    ...
    > "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
echo "accum1 trainer log : $LOG_FILE"
echo "accum1 trainer pid : $(cat "$PID_FILE")"
