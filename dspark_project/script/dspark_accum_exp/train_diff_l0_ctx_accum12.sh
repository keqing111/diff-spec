#!/bin/bash
# Group: diff-attention ctx-only on DRAFT LAYER 0 (0-based), accum=12.
...
RUN_NAME=diff_l0_ctx_a12
RUN_ROOT=/home/y50063564/processed_data/dspark_data/dspark_accum/diff_l0_ctx_a12
...
nohup "$TORCHRUN" --standalone --nproc_per_node 1 \
    ...
    --diff-attention-layer-indices 0 \
    ...
    > "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
echo "diff-l0-ctx-accum12 trainer log : $LOG_FILE"
echo "diff-l0-ctx-accum12 trainer pid : $(cat "$PID_FILE")"
