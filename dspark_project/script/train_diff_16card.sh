#!/bin/bash
# Diff-Transformer 版 DSpark 训练:
#   训练 dp=12(卡 0-11),连 1123 端口的 4 副本 vLLM(卡 12-15)。
#   最后一层(layer_idx=4)用 Diff-Transformer(diffv1) 注意力。
...
# vLLM 与本机同机(80.48.17.178),端口 1123
...
# Ascend NPU assignments (12 个 trainer 用卡 0-11,vLLM 4 副本在卡 12-15)
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/train_diff_$(date +%Y%m%d_%H%M%S).log"
PID_FILE="$LOG_DIR/train_diff.pid"

echo "=== Diff DSpark (12-card): Training on Ascend NPU(s): $TRAIN_NPUS ==="
