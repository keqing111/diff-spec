#!/bin/bash
# 实验: l0_5l + context-only diff 注意力, 5 层 diff@layer0, 单卡 14 训练。
# diff 层只能看 base 上下文(不能看自己合成 block)。
...
OUTPUT_DIR="/home/y50063564/processed_data/dspark_data/dspark_diff_l0_5l_ctxonly"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/train_l0_5l_ctxonly_$(date +%Y%m%d_%H%M%S).log"
PID_FILE="$LOG_DIR/train.pid"

echo "=== l0_5l ctxonly (卡14, diff只看上下文): Training on NPU: $TRAIN_NPUS ==="
