...
def main(args: argparse.Namespace):  # noqa: C901
    # --- aicore 算子执行超时放大(诊断实验;失败静默跳过) ---
    # 默认约 3 分钟,可能误杀"很慢但最终能完成"的 kernel(如 Muon Newton-Schulz 步)。
    # ACL_OP_EXECUTE_TIMEOUT 指定秒数,0=使用硬件允许的最大超时。
    def _enlarge_acl_timeout() -> None:
        try:
            import ctypes
            import os
            ...
            lib.aclrtSetOpExecuteTimeOut.argtypes = [ctypes.c_uint32]
            lib.aclrtSetOpExecuteTimeOut.restype = ctypes.c_int
            secs = int(os.environ.get("ACL_OP_EXECUTE_TIMEOUT", "0"))
            rc = lib.aclrtSetOpExecuteTimeOut(secs)
            print(f"[timeout] aclrtSetOpExecuteTimeOut({secs}) rc={rc}", flush=True)
        except Exception as exc:  # noqa: BLE001
            print(f"[timeout] skip: {exc}", flush=True)

    _enlarge_acl_timeout()
    ...
    # 设备已初始化,重试一次超时放大(首次在 main 顶部若 ACL 未就绪会跳过)
    _enlarge_acl_timeout()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="kv_params",
        choices=["kv_params", "glm52_file"],
        help=(
            "Hidden states transport mode. 'kv_params' (default): use old "
            "ExampleHiddenStatesConnector with .lock files. 'glm52_file': use "
            "Glm52HSDumper, reads hs_<idx>.safetensors with fixed filenames "
            "and no lock files."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--diff-attention-layer-indices",
        type=int,
        nargs="+",
        default=[],
        help="(Optional) Draft layer indices that use Diff-Transformer (diffv1) "
