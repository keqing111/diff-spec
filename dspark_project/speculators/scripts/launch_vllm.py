...
def parse_args():
    ...
    parser.add_argument(
        "--hs-dump-mode",
        type=str,
        default="old_connector",
        choices=["old_connector", "glm52_file"],
        help=(
            "Hidden states dump mode. "
            "'old_connector' (default): use ExampleHiddenStatesConnector with "
            "extract_hidden_states spec decode method + .lock files. "
            "'glm52_file': use GLM52_HS_DUMP env-vars + Glm52HSDumper, "
            "writes hs_<id>.safetensors via tmp+replace (no lock files)."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def main():
    ...
    if args.hs_dump_mode == "glm52_file":
        # GLM52 HS dumper mode: use Glm52HSDumper via environment variables.
        # No speculative_config or kv_transfer_config needed.
        os.environ["GLM52_HS_DUMP"] = "1"
        os.environ["GLM52_HS_DIR"] = args.hidden_states_path
        os.environ["GLM52_HS_LAYER_IDS"] = ",".join(str(x) for x in target_layer_ids)

        disable_cp_arg = "--no-enable-chunked-prefill"
        if disable_cp_arg not in cmd:
            cmd.append(disable_cp_arg)
    else:
        # Old connector mode: use ExampleHiddenStatesConnector with
        # extract_hidden_states spec decode method.
        speculative_config = {
