...
def _json_safe(value):
    ...
    # numpy scalar / torch scalar 一般都有 item()
...
def _hs_profile(event: str, **fields):
    if os.getenv("HS_PROFILE", "0") != "1":
        return

    try:
        path = os.getenv("HS_PROFILE_PATH", "/tmp/hs_profile_train.jsonl")
        record = {
            "ts": time.time(),
            "role": "train",
            "event": event,
        ...
        # profile 代码绝对不能影响训练
        return
_hs_profile("profile_import", module=__file__)
...
class ArrowDataset(BaseDataset):
    def __init__(
        ...
        Args:
            ...
            hs_mode: ``"kv_params"`` (default, old connector path) or ``"glm52_file"``
                (fixed filename hs_<idx>.safetensors, no lock file).
        """
        ...
        try:    # julius
            ...
            if self.hs_mode == "glm52_file":
                ...
                hs_filepath = generate_hidden_states(
                    ...
                    hs_mode="glm52_file",
                    hs_dir=str(self.hidden_states_path),
                    hs_file_idx=file_idx,
                )
                t1 = time.perf_counter()
                _hs_profile("vllm_request", sample=index, dur_s=t1 - t0, path=hs_filepath)
                loaded_hs = self._wait_for_hs_file(Path(hs_filepath))
            else:
                # kv_params mode (default): extract path from response, lock file
                hs_filepath = generate_hidden_states(
                    self.client,  # type:ignore[arg-type]
                    self.model,  # type:ignore[arg-type]
                    client_item,
                ...
                t1 = time.perf_counter()
                _hs_profile("load_generate_file", sample=index, dur_s=t1 - t0, path=hs_filepath,
                            bytes=Path(hs_filepath).stat().st_size if Path(hs_filepath).exists() else None)
            ...
            t1 = time.perf_counter()
            _hs_profile("check_hidden_states", sample=index, dur_s = t1-t0, path=hs_filepath)
            match self.on_generate:
                ...
                case "delete":      # julius
                    ...
                    t1 = time.perf_counter()
                    _hs_profile("unlink_generated_file", sample=index, dur_s = t1-t0, path=hs_filepath)
    ...
    def _wait_for_hs_file(self, path: Path, timeout: float = 900,
                           poll_interval: float = 0.5) -> dict[str, torch.Tensor]:
        """Poll for a fixed-path safetensors file (glm52_file mode).

        No lock file is used — the dumper writes via ``tmp -> os.replace``
        ...
        while time.monotonic() < deadline:
            if path.exists():
                try:
                    ...
                    # (shouldn't happen with os.replace, but be safe).
                    time.sleep(0.1)
                    continue
            now = time.monotonic()
            if now - last_log > 30:
                logger.info("Waiting for %s ... (%.0fs left)", path, deadline - now)
                last_log = now
            time.sleep(poll_interval)
        raise TimeoutError(
            f"Timed out after {timeout}s waiting for {path}. "
            "Check that the vLLM server is running and GLM52_HS_DIR is correct."
        )
    ...
    def _get_raw_data(self, index):
        ...
        t1 = time.perf_counter()
        _hs_profile("load_cached_candidate", sample=index, dur_s = t1-t0, path=str(candidate_path),
        hit=loaded_hs is not None, bytes=candidate_path.stat().st_size if candidate_path.exists() else None)
        ...
        if hs.dim() == 4 and hs.shape[1] == 2:
            # Connector format: [seq, 2(KV), num_layers, hidden_size]
            # Take K only and slice out the last layer (appended by
            # include_last_layer in launch_vllm.py) as verifier_last.
            hs = hs[:, 0]  # [seq, num_layers, hidden_size]
        return {
            "hidden_states": hs[:, :-1].flatten(1),  # [seq, (num_layers-1)*hidden_size]
