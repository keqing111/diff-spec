...
def extract_output_glm52(
    response: Completion | ChatCompletion,
    token_ids: list[int],
    hs_dir: str,
    hs_file_idx: int,
) -> str:
    """Extract hidden states path for glm52_file mode.

    Validates prompt_token_ids, then returns a fixed file path
    based on ``hs_dir`` and ``hs_file_idx``. No ``kv_transfer_params``
    needed — the GLM5.2 dumper writes to a known filename.
    """
    if isinstance(response, Completion):
        prompt_token_ids = getattr(response.choices[0], "prompt_token_ids", None)
    else:
    ...
    return os.path.join(hs_dir, f"hs_{hs_file_idx}.safetensors")
...
async def generate_hidden_states_async(
    ...
    *,
    ...
    Args:
        ...
        hs_dir: Hidden states output directory (required when hs_mode="glm52_file").
        hs_file_idx: Dataset file index (required when hs_mode="glm52_file").
    """
    token_ids = client_item["input_ids"]
    messages = client_item.get("messages")

    extra_kwargs = {}
    if hs_mode == "glm52_file":
        if hs_dir is None or hs_file_idx is None:
            raise ValueError(
                "hs_dir and hs_file_idx are required when hs_mode='glm52_file'"
            )
        extra_kwargs["extra_headers"] = {"X-Request-Id": f"hs_{hs_file_idx}"}
    ...
    if hs_mode == "glm52_file":
        return extract_output_glm52(res, token_ids, hs_dir, hs_file_idx)
