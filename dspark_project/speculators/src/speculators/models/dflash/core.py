...
class DFlashDraftModel(DraftVocabMixin, SpeculatorModel):
    ...
    def __init__(
        ...
        # Layers listed in diff_attention_layer_indices use Diff-Transformer
        # (diffv1) attention; all others keep the standard GQA attention. The
        # default (empty list) reproduces the previous behavior exactly.
        diff_layer_indices = set(config.diff_attention_layer_indices or [])
        if any(idx < 0 or idx >= num_draft_layers for idx in diff_layer_indices):
            raise ValueError(
